...
def reducir_suficientemente(M):
    """
    Aplica todas las reducciones suficientes posibles a la matriz del canal M.
    Retorna la matriz reducida.
    """
    seguir_reduciendo = True

    while seguir_reduciendo:
        seguir_reduciendo = False
        m = len(M[0])
        c1 = 0

        # Recorremos hasta que encontremos una reducción o se terminen las columnas
        while c1 < m - 1 and not seguir_reduciendo:
            c2 = c1 + 1
            while c2 < m and not seguir_reduciendo:

                if columnas_se_pueden_combinar(M, c1, c2):

                    # Reducimos
                    _, M = reducir_columnas_con_determinante(M, c1, c2)

                    # Indica que debemos recomenzar con la nueva matriz
                    seguir_reduciendo = True
...
M_reducida = reducir_suficientemente(M)
